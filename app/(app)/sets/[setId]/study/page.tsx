...
export default function StudyPage() {
  ...
  const cards = useMemo(() => {
    const eligibleCards = (set?.cards ?? []).filter((card) => filter === "all" || card.status === "learning");
    const cardsById = new Map(eligibleCards.map((card) => [card.id, card]));
    const orderedCards = order.flatMap((cardId) => {
      const card = cardsById.get(cardId);
      return card ? [card] : [];
    });
    const orderedIds = new Set(order);
    return [...orderedCards, ...eligibleCards.filter((card) => !orderedIds.has(card.id))];
  }, [filter, order, set]);
  ...
  const toggleFilter = () => {
    setFilter((value) => value === "all" ? "learning" : "all");
    setIndex(0);
    setFlipped(false);
    setDone(false);
  };
  const mark = async (status: "learning" | "learned") => {
    if (!current) return;
    const updated = await updateCard(current.id, { status });
    setSet((value) => value ? { ...value, cards: value.cards.map((card) => card.id === updated.id ? updated : card) } : value);
    setMarked((value) => ({ ...value, [status]: value[status] + 1 }));

    if (filter === "learning" && status === "learned") {
      setFlipped(false);
