...
import { GraduationCap, MoreHorizontal, Pencil, Pin, Plus, Search, Trash2 } from "lucide-react";
...
export default function SetsPage() {
  ...
  const togglePin = async (set: VocabularySet) => {
    try {
      ...
      setSets((prev) => [...prev.map((item) => item.id === updated.id ? updated : item)].sort((a, b) => Number(b.is_pinned) - Number(a.is_pinned)));
    } catch (cause) {
      setError(getErrorMessage(cause));
    }
  };
  ...
  return (
    <AppShell title={t("sets")} action={<Link href="/sets/new"><Button size="sm"><Plus className="h-4 w-4" />{t("newSet")}</Button></Link>}>
      ...
      <div className="relative mb-5"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("searchSets")} className="pl-9" /></div>
